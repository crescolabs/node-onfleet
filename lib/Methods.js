...
const Methods = (key, api, ...args) => {
  const { path, altPath, method, queryParams, timeoutInMilliseconds } = key;
  ...
  if (operations === "POST") {
    if (util.isBase64Encoded(args[0])) {
      // forceComplete and clone (with ID)
      url = util.replaceWithId(url, args[0]);
      if (args[1]) {
        // forceComplete (with ID, has body)
        body = args[1]; // eslint-disable-line
        hasBody = true;
      }
    } else {
      // create, batchCreate, and autoAssign (no ID, has body)
